...
description: How to configure environment variables in next-ship.
---

next-ship uses environment variables for configuration. This guide will help you set up the required variables to get started quickly, and optionally configure additional features.

## Quick Start (Minimum Setup)

To get next-ship running locally with basic functionality, you only need to configure these **required** variables:
...
<Step>
  Fork the [next-ship template](https://basehub.com/basehub/next-ship?fork=1) on BaseHub
</Step>
...
<Step>
  Configure your webhook endpoint in Polar and set `POLAR_WEBHOOK_SECRET`.
</Step>
...
next-ship uses environment variables across multiple locations:
