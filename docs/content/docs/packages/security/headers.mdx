...
next-ship uses [Nosecone](https://docs.nosecone.com/quick-start) to set HTTP response headers related to security.
...
See the [Nosecone reference](https://docs.nosecone.com/reference) for details on each header and configuration options.
...
The CSP header is not set by default because it requires specific configuration based on the next-ship features you have enabled.

In the meantime, you can set the CSP header using the Nosecone configuration. For example, the following CSP configuration will work with the default next-ship features:
