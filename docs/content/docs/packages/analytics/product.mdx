...
next-ship has support for product analytics via PostHog — a single platform to analyze, test, observe, and deploy new features.
...
To automatically capture authentication and payment events, we've combined PostHog's Node.js server-side library with Clerk and Polar webhooks to wire it up as follows:
